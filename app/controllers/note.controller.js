...
exports.findOne = function(req, res){
  ...
  Note.findById(req.params.noteId, function(err, data){
    if(err){
      res.status(500).send({
        message: "Could not retreive note with ID: " + req.params.noteId
      });
    } else{
      res.send(data);
    }
...
exports.update = function(req, res){
  ...
  Note.findById(req.params.noteId, function(err, note){
    if(err){
      res.status(500).send({
        message: "Could not find note with ID: " + req.params.noteId
      });
    }
...
exports.delete = function(req, res){
  // Delete a note with the specified noteId in the requests
  Note.remove({__id: req.params.noteId}, function(err, data){
    if(err){
      res.status(500).send({
